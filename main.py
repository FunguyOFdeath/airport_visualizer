...
def rabbitmq_listener(input_queue):
    ...
    while True:  # Бесконечный цикл для переподключения
        try:
            ...
            parameters.heartbeat = 600  # Увеличиваем heartbeat до 10 минут
            parameters.blocked_connection_timeout = 300
            parameters.retry_delay = 5
            connection = pika.BlockingConnection(parameters)
            channel = connection.channel()
            # Объявляем очереди (durable=False)
            ...
            logger.error(f"Ошибка подключения или канала: {e}")
...
def main():
    ...
    car_factor = 0.1  # Машины в 2 раза меньше самолёта
    ...
    while running:
            ...
            if cmd_line.startswith("/"):
                ...
                if parts[0] == "/way":
                    ...
                    if removed is not None:
                        print("Команда очистки выполнена успешно")
                else:
        ...
        for plane_id, plane_data in list(comands.planes.items()):
            ...
            if idx < len(route):
                ...
                # Если текущая или целевая точка — ребро, используем середину ребра
                ...
                if dist < plane_data["speed"]:
                    ...
                    # Если остановились на вершине (не ребре), можно установить базовый угол
                    if not comands.is_edge(target_vertex):
                        plane_data["last_angle"] = 0
                else:
        ...
        for car_id, car_data in list(comands.cars.items()):
            ...
            if idx < len(route):
                ...
                computed_angle = math.degrees(math.atan2(-dy, dx)) + 180
                # Всегда сохраняем угол в направлении движения
                car_data["last_angle"] = computed_angle
            else:
                if route and route[-1] == car_data.get("start_origin"):
                    comands.car_counts[model] -= 1
                    del comands.cars[car_id]
                    continue
                computed_angle = car_data.get("last_angle", 0)
            ...
            if car_id in comands.cars:
                ...
                if model in comands.car_images_scaled and comands.car_images_scaled[model]:
                    ...
                    rotated_image = pygame.transform.rotate(car_image, car_data.get("last_angle", 0))
